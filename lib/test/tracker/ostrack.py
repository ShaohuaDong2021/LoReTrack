...
class OSTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        # here we change the resoulution in test process
        ...
        # kernel = torch.tensor(
        #     [torch.exp(torch.tensor(-(x - kernel_size // 2) ** 2 / (2 * sigma ** 2))) for x in range(kernel_size)]).cuda()
        # kernel = kernel / kernel.sum().cuda()
        # # Apply the Gaussian smoothing along both spatial dimensions
        # self.output_window = torch.nn.functional.conv2d(self.output_window, kernel.view(1, 1, kernel_size, 1).repeat(1, 1, 1, kernel_size),
        #                            padding=kernel_size // 2).cuda()

        # here is the output_window
        # output_window_2 = torch.log10(self.output_window)
        # min_val = torch.min(output_window_2)
        # max_val = torch.max(output_window_2)
        # self.output_window = (output_window_2 - min_val) / (max_val - min_val)



        # hanning = np.hanning(16)
        # self.window = np.outer(hanning, hanning)
        # self.window = torch.from_numpy(self.window).cuda()
    ...
    def track(self, image, info: dict = None):
        ...
        # print(self.output_window, "00000000000000000")
        # print(pred_score_map, "3333333333333333333")
        # print(response, "1111111111111111111")

        # this use the cos (73.4 for got-10k dataset)
        # response = pred_score_map * (1 - self.cfg.TEST.window) + self.output_window * self.cfg.TEST.window

        # this is our window (this one is not good)
        # response = pred_score_map * (1 - self.cfg.TEST.window) + self.window * self.cfg.TEST.window
...
#         self.window_influence = self.cfg.TEST.window
#         self.lr = 0.616
# 
#         # for debug
#         self.debug = params.debug
...
# 
#                     s_c[idx_x, idx_y] = self.change(
#                         self.sz(mapped_box[2], mapped_box[3]) / self.sz_wh(self.state[2:]))  # size
#                     r_c[idx_x, idx_y] = self.change(
#                         (self.state[2] / self.state[3]) / (mapped_box[2] / mapped_box[3]))  # aspect-ratio
...
#         self.lr = 0.616
#
#         # self.penalty_k = 0.000
#         # self.window_influence = 0.300
#         # # self.lr = 0.313
#         # self.lr = 0.0
#
#         # for debug
