...
def warp_boxes(bboxes, grid):
    ...
    x_idx = torch.searchsorted(grid[..., 0].contiguous(),
                               points[:, None, :, 0].expand(-1, h, -1).contiguous())  # (bs,h,2n)
    ...
    x_idx_final = torch.cat([x_idx, x_idx[:, -1:, :]], dim=-2).gather(dim=1, index=y_idx.permute(0, 2, 1)).permute(0, 2,
                                                                                                                   1)  # (bs, 2n, 1)
    ...
    grid_point = flatten_grid.expand(-1, 2 * n, -1, -1).gather(dim=-2, index=idx.unsqueeze(-1).expand(-1, -1, -1,
                                                                                                      2))  # (bs, 2n, topk, 2)
    p1 = grid_point[:, :, 0, :]
    p2 = grid_point[:, :, 1, :]
    p = points.squeeze(2)
...
class QPGrid():
    ...
    def precompute_ind(self):
        ...
        self.asap_k_col = np.concatenate(
            [np.arange(m, dtype=int).repeat(n), np.tile((np.arange(m, m + n, dtype=int)), m)])
        ...
        self.naive_constrain_A_np = np.stack(
            [np.concatenate([np.ones(m), np.zeros(n)]), np.concatenate([np.zeros(m), np.ones(n)])])
    ...
    def bbox2sal(self, batch_bboxes, img_metas, jitter=None):
        ...
        for i in range(len(img_metas)):
            ...
            sal = self.amplitude_scale * (
                        sal / (0.00001 + sal.sum(axis=1, keepdims=True)))  # noqa: E501, normalize each distribution
    ...
    def asap_ts_weight(self, saliency, im_shape, loss_list):
        ...
        q = np.concatenate(
            [sal_x * (-2 * h / m / self.zoom_factor * w_ts), sal_y * (-2 * w / n / self.zoom_factor * w_ts)])[:, None]
        return P, q
    ...
    def ts_loss(self, saliency, im_shape):
        ...
        r_bottom = np.concatenate(
            [np.zeros((m * n, m)), np.concatenate([np.diag(saliency[i, :]) for i in range(m)], axis=0)], axis=1)
...
def build_data_search_grid_generator(cfg):
    loss_dict = dict()
    for loss_name, loss_weight in zip(cfg.DATA.SEARCH.GRID.GENERATOR.LOSS.NAMES,
                                      cfg.DATA.SEARCH.GRID.GENERATOR.LOSS.WEIGHTS):
