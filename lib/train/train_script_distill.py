...
def run(settings):
    ...
    config_module = importlib.import_module("lib.config.%s.config_dissilation" % settings.script_name)
    ...
    if settings.script_name == "ostrack":
        net, net_teacher = build_ostrack(cfg)
    ...
    if settings.script_name == "ostrack.py":
        ...
        actor = DISActor(net=net, net_teacher=net_teacher, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
