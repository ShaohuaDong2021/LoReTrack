...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python lib/train/run_distillation.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_port %d lib/train/run_distillation.py " \
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_distillation.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
